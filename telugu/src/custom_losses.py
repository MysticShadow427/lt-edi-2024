...
class SupConWithCrossEntropy(nn.Module):
    def __init__(self,temperature=0.07, base_temperature=0.07,weight = 0.2):
        ...
        self.temperature = temperature
        self.base_temperature = base_temperature
        self.supcon = SupervisedContrastiveLoss(self.temperature,self.base_temperature)
        self.cross_entropy = nn.CrossEntropyLoss()
        self.weight = weight

    def forward(self,features,logits,labels):
        return self.weight * self.supcon(features,labels) + (1-self.weight)*self.cross_entropy(logits,labels)
